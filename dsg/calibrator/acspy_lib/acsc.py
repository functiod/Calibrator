...
import ctypes
from ctypes import byref, c_double, create_string_buffer
import numpy as np
import platform
import re
...
if platform.architecture()[0] == "32bit":
    # acs = ctypes.windll.LoadLibrary(r"C:\Windows\System32\ACSCL_x86.dll")
    acs = ctypes.windll.LoadLibrary("ACSCL_x86.dll")
if platform.architecture()[0] == "64bit":
    # acs = ctypes.windll.LoadLibrary(r"C:\Windows\System32\ACSCL_x64.dll")
...
ACSC_SAFETY_NETWORK = 4
ACSC_SAFETY_HOT = 0x00000010
ACSC_SAFETY_SRL = 0x00000020
ACSC_SAFETY_SLL = 0x00000040
ACSC_SAFETY_ENCNC = 0x00000080
ACSC_SAFETY_ENC2NC = 0x00000100
ACSC_SAFETY_DRIVE = 0x00000200
ACSC_SAFETY_ENC = 0x00000400
ACSC_SAFETY_ENC2 = 0x00000800
ACSC_SAFETY_PE = 0x00001000
ACSC_SAFETY_CPE = 0x00002000
ACSC_SAFETY_VL = 0x00004000
ACSC_SAFETY_AL = 0x00008000
ACSC_SAFETY_CL = 0x00010000
ACSC_SAFETY_SP = 0x00020000
ACSC_SAFETY_PROG = 0x02000000
ACSC_SAFETY_ES = 0x10000000
ACSC_SAFETY_INT = 0x20000000
...
def disableResponse(hcomm, axis, response, wait=SYNCHRONOUS):
    call_acsc(acs.acsc_DisableResponse, hcomm, int32(axis), response, wait)

def enableResponse(hcomm, axis, response, wait=SYNCHRONOUS):
    call_acsc(acs.acsc_EnableResponse, hcomm, int32(axis), response, wait)
...
def setFPosition(hcomm, axis, FPosition, wait=SYNCHRONOUS):
    call_acsc(acs.acsc_SetFPosition, hcomm, axis, double(FPosition), wait)

def getFPosition(hcomm: int, axis: int, wait=SYNCHRONOUS):
    result: c_double = double(0)
    call_acsc(acs.acsc_GetFPosition, hcomm, axis, ctypes.pointer(result), wait)
    return result.value

def getRVelocity(hcomm, axis, wait: int | None = SYNCHRONOUS):
    rvel = double()
...
def acs_Break(hcomm, axis, wait=SYNCHRONOUS):
    call_acsc(acs.acsc_Break, hcomm, int32(axis), wait)
