...
def getMlxPhoto(set):
    ...
    filename = time_now.strftime("dsg\calibrator\PHOTO\\%Y-%m-%d %H-%M-%S IRphoto ")
...
def getPhoto(set):
    ...
    saveImageToFile(getImg8bit(set), set)
...
#    text_draw.text((3, h_text * 8), "Sens address:  %d"   % addr_rec, (255, 255, 255), font=text_font)
    ...
    filename = datetime.datetime.now()
    img.save(filename.strftime("dsg\calibrator\PHOTO\\%Y-%m-%d %H-%M-%S photo.png"))
